  ...
  <div class="space-y-4 sm:space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-black/10 shadow-sm p-4 sm:p-6 lg:p-8">
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
        ...
        <div v-if="unreadCount > 0" class="px-3 sm:px-4 py-1.5 sm:py-2 bg-[#006AC7] text-white rounded-xl font-semibold text-sm sm:text-base whitespace-nowrap">
    ...
    <!-- Inbox -->
    <div class="bg-white rounded-2xl border border-black/10 shadow-sm p-4 sm:p-6 lg:p-8 space-y-4">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <h3 class="text-lg sm:text-xl font-bold text-[#0B2A3C]">{{ t("clientFAQ.inbox.title") }}</h3>

        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
          <input
            v-model="searchQuery"
            type="text"
            class="w-full sm:w-auto px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-black/10 bg-white text-sm sm:text-base text-[#0B2A3C] focus:outline-none focus:ring-2 focus:ring-[#006AC7]/20 focus:border-[#006AC7]"
            :placeholder="t('clientFAQ.inbox.searchPlaceholder')"
          />

          <select
            v-model="filterStatus"
            class="w-full sm:w-auto px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-black/10 bg-white text-sm sm:text-base text-[#0B2A3C]"
          >
      ...
      <div v-else class="space-y-3">
        <div
          ...
          :key="item.id"
          class="p-3 sm:p-4 rounded-xl border border-black/10 hover:border-[#006AC7]/30 hover:shadow-sm transition"
        >
          <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
            <div class="flex-1 cursor-pointer min-w-0" @click="openDetail(item)">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <div class="text-sm sm:text-base font-bold text-[#0B2A3C] break-words flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
              <button
                ...
                class="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg border border-black/10 hover:bg-black/5 text-xs sm:text-sm font-semibold whitespace-nowrap"
              >
                {{ item.is_read ? t("clientFAQ.inbox.markUnread") : t("clientFAQ.inbox.markRead") }}
              </button>

              <button
                @click.stop="deleteMsg(item)"
                class="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg border border-[#B42318]/30 text-[#B42318] hover:bg-[#FFE0E0] text-xs sm:text-sm font-semibold whitespace-nowrap"
              >
                {{ t("common.delete") }}
              </button>
            </div>
        ...
        <div v-if="totalPages > 1" class="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-black/10">
          <button
    ...
    >
      <div class="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col">
        <div class="p-4 sm:p-6 border-b border-black/10 flex items-center justify-between gap-3">
          <div class="text-lg sm:text-xl font-bold text-[#0B2A3C] break-words flex-1 min-w-0">
            {{ selected.title || t("clientFAQ.inbox.noTitle") }}
          </div>
          <button @click="closeDetail" class="text-[#6B7E8B] hover:text-[#0B2A3C] text-xl sm:text-2xl flex-shrink-0 w-8 h-8 flex items-center justify-center">✕</button>
        </div>

        <div class="p-4 sm:p-6 overflow-y-auto">
          <div class="text-xs text-[#6B7E8B] mb-4">{{ formatDate(selected.created_at) }}</div>
          <div class="whitespace-pre-wrap text-sm sm:text-base text-[#0B2A3C] bg-[#F7FBFF] rounded-xl p-3 sm:p-4">
            {{ selected.message }}
          </div>
        </div>
