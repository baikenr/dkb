...
const loadUnreadCount = async () => {
  if (appStore.authType !== "staff") return;
  try {
    const res = await appStore.staffNotificationCount();
...
watch(() => route.path, (newPath) => {
  if (newPath === "/staff/faq" || newPath.includes("/staff/faq")) {
    loadUnreadCount();
  }
});
</script>
